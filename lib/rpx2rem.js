...
var defaultConfig = {
  ...
  forcePxComment: 'rpx',   // force rpx comment (default: `rpx`)
        ...
        if (declaration.type === 'declaration' && rpxRegExp.test(declaration.value)) {
          ...
          if (nextDeclaration && nextDeclaration.type === 'comment') { // next next declaration is comment
            if (nextDeclaration.comment.trim() === config.keepComment) { // no transform
              ...
              continue;
            } else if (nextDeclaration.comment.trim() === config.forcePxComment) { // force rpx
              declarations.splice(j + 1, 1); // delete corresponding comment
            }
          }
          declaration.value = self._getCalcValue('rpx', declaration.value, dpr); // common transform
        }
      }
    }
  }
...
Px2rem.prototype.generateRem = function (cssText) {
  ...
  function processRules(rules, noDealPx) { // FIXME: keyframes do not support `force rpx` comment
    for (var i = 0; i < rules.length; i++) {
      var rule = rules[i];
      if (rule.type === 'media') {
        processRules(rule.rules); // recursive invocation while dealing with media queries
      ...
      for (var j = 0; j < declarations.length; j++) {
        ...
        if (declaration.type === 'declaration' && rpxRegExp.test(declaration.value)) {
          ...
          if (nextDeclaration && nextDeclaration.type === 'comment') { // next next declaration is comment
            if (nextDeclaration.comment.trim() === config.forcePxComment) { // force rpx
              ...
              if (!noDealPx) {
                ...
                for (var dpr = 1; dpr <= 3; dpr++) {
                  ...
                  newDeclaration.value = self._getCalcValue('rpx', newDeclaration.value, dpr);
                  newRules[dpr - 1].declarations.push(newDeclaration);
                }
                declarations.splice(j, 2); // delete this rule and corresponding comment
                j--;
              } else { // FIXME: keyframes do not support `force rpx` comment
      ...
      if (!noDealPx) {
        // add the new rules which contain declarations that are forced to use rpx
...
// get calculated value of rpx or rem
Px2rem.prototype._getCalcValue = function (type, value, dpr) {
  var config = this.config;
  var rpxGlobalRegExp = new RegExp(rpxRegExp.source, 'g');
  ...
  return value.replace(rpxGlobalRegExp, function ($0, $1) {
    return type === 'rpx' ? getValue($1 * dpr / config.baseDpr) : getValue($1 / config.remUnit);
  });
};
